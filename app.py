    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(user.password, form.password.data):
            if user.is_active:
                ...
                # Redirect to the 'next' page if it exists, otherwise redirect to /dash
                next_page = request.args.get('next')
                return redirect(next_page) if next_page else redirect(url_for("dash_app_route"))
...
def register():
    ...
    if form.validate_on_submit():
        ...
        msg = Message('Confirm your Email', sender='seyeoyelayo@gmail.com', recipients=[form.email.data])
